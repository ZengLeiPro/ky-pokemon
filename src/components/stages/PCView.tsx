...
const PCView: React.FC = () => {
  const { playerParty, playerStorage, depositPokemon, withdrawPokemon, setView } = useGameStore();
                  ...
                  {playerParty.map((p, idx) => (
                      ...
                      >
                          ...
                          <div className="flex-1 text-left min-w-0">
                              <div className="text-sm font-bold text-slate-200 truncate">{p.nickname || p.speciesName}</div>
          ...
          <div className="flex-1 p-4 overflow-y-auto">
              ...
              {playerStorage.length === 0 ? (
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                      {playerStorage.map((p, idx) => (
                          <button
                            key={p.id}
                            onClick={() => withdrawPokemon(p.id)}
                          ...
                          >
                               ...
                               <span className="text-[10px] font-medium text-slate-400 truncate max-w-full px-1 relative z-10">{p.nickname || p.speciesName}</span>
