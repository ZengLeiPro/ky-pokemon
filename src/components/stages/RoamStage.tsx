...
import { Compass, Moon, Navigation, ShoppingBag, House, Heart, Swords } from 'lucide-react';
import { WORLD_MAP, SPECIES_DATA } from '../../constants';

const RoamStage: React.FC = () => {
  const { startBattle, startGymBattle, healParty, addLog, addItem, playerLocationId, moveTo, buyItem, playerMoney } = useGameStore();
        ...
        <div className="z-10 w-full h-full overflow-y-auto flex flex-col items-center p-6 pb-32">
            ...
            <div className="grid grid-cols-2 gap-4 w-full max-w-xs shrink-0">
                 <button 
                    ...
                    className={`bg-indigo-600/90 hover:bg-indigo-600 active:bg-indigo-700 text-white p-4 rounded-2xl shadow-lg border-b-4 border-indigo-800 active:border-b-0 active:translate-y-1 transition-all flex flex-col items-center justify-center gap-2 group ${!isTown ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
                >
                    <House size={20} className="group-hover:scale-110 transition-transform" />
                    <span className="text-xs font-bold">宝可梦中心</span>
                </button>
                ...
                <button
                    ...
                    className={`bg-amber-600/90 hover:bg-amber-600 active:bg-amber-700 text-white p-4 rounded-2xl shadow-lg border-b-4 border-amber-800 active:border-b-0 active:translate-y-1 transition-all flex flex-col items-center justify-center gap-2 group ${!isTown ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
                >
                ...
                <button
                    ...
                    className={`col-span-2 bg-rose-600/90 hover:bg-rose-600 active:bg-rose-700 text-white p-4 rounded-2xl shadow-lg border-b-4 border-rose-800 active:border-b-0 active:translate-y-1 transition-all flex flex-col items-center justify-center gap-2 group ${!hasGym ? 'opacity-50 grayscale cursor-not-allowed' : ''}`}
                >
                    <Swords size={20} className="group-hover:scale-110 transition-transform" />
                    <span className="text-xs font-bold">宝可梦道馆</span>
                </button>
            </div>
        </div>
