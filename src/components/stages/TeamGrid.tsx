...
import { Star, HardDrive, Smartphone, Undo2 } from 'lucide-react';
...
const TeamGrid: React.FC = () => {
  ...
  const { playerParty, setView, setSelectedPokemon, battle, addLog, swapPartyPosition } = store;
  ...
  // 操作菜单状态：记录当前打开菜单的宝可梦索引
  const [menuIndex, setMenuIndex] = React.useState<number | null>(null);
  // 交换模式状态：记录发起交换的宝可梦索引
  const [swapSourceIndex, setSwapSourceIndex] = React.useState<number | null>(null);

  // 点击宝可梦卡片
  const handleCardClick = (idx: number) => {
    // 如果处于交换模式，点击目标完成交换
    if (swapSourceIndex !== null) {
      if (idx !== swapSourceIndex) {
        swapPartyPosition(swapSourceIndex, idx);
      }
  ...
  const handleMenuSummary = () => {
    ...
    const pokemon = playerParty[menuIndex];
  ...
  const handleMenuSwap = () => {
    if (menuIndex === null) return;
    setSwapSourceIndex(menuIndex);
    setMenuIndex(null);
  };

  // 菜单操作：传送到设备
  const handleMenuTransfer = async () => {
    if (menuIndex === null) return;
    const pokemon = playerParty[menuIndex];
    setMenuIndex(null);
    await handleTransfer(pokemon);
  };

  const handleTransfer = async (pokemon: Pokemon) => {
  ...
  return (
    <div className="h-full bg-slate-950 p-4 flex flex-col" style={{ touchAction: 'pan-y' }}>
       ...
       {swapSourceIndex !== null && (
         <div className="mb-4 bg-cyan-500/10 p-3 rounded-xl border border-cyan-500/30 text-center">
           <span className="text-cyan-400 text-sm font-bold">
             选择要交换的宝可梦
           </span>
           <button
             onClick={() => setSwapSourceIndex(null)}
             className="ml-3 text-xs text-slate-400 hover:text-white underline"
           >
             取消
           </button>
         </div>
       ...
       {devicePkm && (
        ...
        <div className="grid grid-cols-1 gap-4 overflow-y-auto pb-4">
            {playerParty.map((pokemon, idx) => {
                // 交换模式下：源宝可梦半透明，其他高亮可点击
                const isSwapSource = swapSourceIndex === idx;
                const isSwapTarget = swapSourceIndex !== null && swapSourceIndex !== idx;

                return (
                  <div
                    key={pokemon.id}
                    onClick={() => handleCardClick(idx)}
                    className={`bg-slate-900 p-4 rounded-2xl border flex gap-4 items-center text-left transition-all active:scale-[0.98] shadow-lg relative cursor-pointer group ${
                      idx === 0 ? 'border-emerald-500/50' : 'border-slate-800'
                    } ${isSwapSource ? 'opacity-50 ring-2 ring-cyan-400' : ''} ${
                      isSwapTarget ? 'hover:bg-cyan-900/30 hover:border-cyan-500/50' : 'hover:bg-slate-800'
                    }`}
                  >
                    {/* 首发标签 */}
                    {idx === 0 && (
                    ...
                    {transferring === pokemon.id && (
                      <div className="absolute inset-0 bg-yellow-500/10 rounded-2xl animate-pulse z-5" />
                    )}
       ...
       {/* 操作菜单弹窗 */}
       {menuIndex !== null && playerParty[menuIndex] && (
         <div
           className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm"
           onClick={() => setMenuIndex(null)}
         >
           <div
             className="bg-slate-900/95 border border-slate-700 rounded-2xl p-5 w-64 shadow-2xl"
             onClick={(e) => e.stopPropagation()}
           >
             {/* 菜单标题 */}
             <div className="text-center mb-4">
               <span className="text-white font-bold text-lg">
                 {playerParty[menuIndex].nickname || playerParty[menuIndex].speciesName}
               </span>
             </div>
             ...
             <div className="flex flex-col gap-2">
               ...
               <button
                 ...
                 className="w-full py-3 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-sm transition-colors"
               >
                 详细信息
               </button>
               ...
               <button
                 ...
                 disabled={playerParty.length <= 1 || transferring !== null}
                 className={`w-full py-3 px-4 rounded-xl font-bold text-sm transition-colors ${
                   playerParty.length <= 1 || transferring !== null
                     ? 'bg-slate-800/50 text-slate-600 cursor-not-allowed'
               ...
               <button
                 onClick={() => setMenuIndex(null)}
                 className="w-full py-3 px-4 rounded-xl bg-slate-800/50 hover:bg-slate-700 text-slate-400 font-bold text-sm transition-colors mt-1"
               >
                 取消
               </button>
             </div>
